...
public protocol ContextAssembling: Sendable {
    func assembleContext(perception: Perception) async throws -> ContextBundle
}
...
public struct DefaultContextAssembler: ContextAssembling {
    public init() {}
    public func assembleContext(perception: Perception) async throws -> ContextBundle {
...
public actor M6Orchestrator {
    ...
    public func run(goalID: GoalID, rawInput: String? = nil) async throws -> Evaluation {
        ...
        while cycleCount < maxCycles {
            ...
            // 2. Context
            let context = try await contextAssembler.assembleContext(perception: perception)
            try await emit(
                traceID: traceID,
                kind: .contextBuilt,
            ...
            for proposal in proposals {
                // EXPLICIT toolID: proposal.toolID passed directly without inference!
    ...
    private func executeProposal(_ proposal: ActionProposal, traceID: TraceID, goalID: GoalID) async throws -> Observation {
        ...
        if let toolID = proposal.toolID, let modules = moduleRuntime {
            let contracts = await modules.contracts()
            let raw = toolID.rawValue
            let targetContract = contracts.first(where: { $0.id.rawValue == raw })
                ?? contracts.first(where: { raw.hasPrefix("tool.") && $0.id.rawValue == String(raw.dropFirst(5)) })
                ?? contracts.first(where: { !raw.hasPrefix("tool.") && $0.id.rawValue == "tool.\(raw)" })

            let targetID = targetContract?.id.rawValue ?? raw
            let schema = targetContract?.inputSchema ?? SchemaDocument(identifier: "text")
            var fields = ["input": proposal.description, "message": proposal.description]
            ...
            let invocation = ModuleInvocation(
                moduleID: ModuleID(rawValue: targetID),
                input: ModulePayload(schema: schema, fields: fields)
            )

            do {
                let res = try await modules.execute(invocation)
                obsSummary = res.output.fields["output"] ?? res.output.fields["result"] ?? res.output.fields["message"] ?? "Success"
                succeeded = true
            } catch {

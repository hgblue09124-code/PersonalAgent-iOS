...
public actor RunRecoveryEngine {
    ...
    public func recoverRun(runID: RunID) async throws -> RecoveryOutcome {
        ...
        for var attempt in startedUnknownAttempts {
            ...
            case .unknown, .unavailable:
                ...
                if let toolID = attempt.toolID {
                    let cap = await executionBoundary.targetCapability(for: toolID)
    ...
    public func recoverStateJournal(runID: RunID, traceID: TraceID) async throws {
        ...
        for var entry in pending {
            switch entry.status {
            case .prepared:
                // Reconcile against AgentRuntime as sole authority
                let goal = await runtime.goal(id: entry.goalID)
                if goal?.status == entry.targetStatus {
                    ...
                    let payload = Self.parseCanonicalPayload(entry.canonicalEventPayload)

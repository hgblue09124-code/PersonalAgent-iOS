...
public actor ExecutionBoundary {
            ...
            do {
                ...
                if executorSucceeded {
                    ...
                    case .notStarted:
                        ...
                        try await attemptStore.saveAttempt(attempt)
                        let obs = Observation(actionID: proposal.actionID, summary: "Execution unverified (not started)", succeeded: false)
                        return (obs, attempt)

                    case .unknown, .unavailable:
                        let cap = targetCapability(for: toolID)
                        ...
                        try await attemptStore.saveAttempt(attempt)
                        let obs = Observation(actionID: proposal.actionID, summary: "Execution unverified (\(verificationResolution)): \(obsSummary)", succeeded: false)
                        return (obs, attempt)
                    }
                } else {
    ...
    public func resolveEvidence(
        ...
        guard cap.supportsEvidenceResolution else {
            // Target does NOT support evidence resolution: global resolver MUST NOT be used!
            return .unavailable
        }

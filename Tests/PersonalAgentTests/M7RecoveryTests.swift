...
struct M7RecoveryTests {
    ...
    func testStateJournalRecoveryWhenStateAppliedBeforeCrash() async throws {
        ...
        // Apply state update to AgentRuntime first with required evidence
        ...
        let entry = StateJournalEntry(
            ...
            targetStatus: .completed,
            eventID: eventID,
            canonicalEventPayload: "disposition=complete&goalID=\(goal.id.rawValue)&reason=done&targetStatus=completed",
            status: .prepared
        )
    ...
    func testStateJournalRecoveryWhenStateNotAppliedBeforeCrash() async throws {
        ...
        // Inject journal entry in .prepared status, BUT AgentRuntime goal is still .active
        let entry = StateJournalEntry(
            runID: record.runID,
            goalID: goal.id,
            targetStatus: .completed,

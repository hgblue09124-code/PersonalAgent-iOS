...
struct M1EventDeterminismTests {
    @Test func startEmitsInitializedThenStarted() async throws {
        ...
        let runtime = try await AgentRuntime(identity: AgentIdentity(id: AgentID(rawValue: "e1"), displayName: "E"), eventLog: log, sessionTrace: TraceID(rawValue: "trace.1"))
    ...
    @Test func identicalSequencesProduceIdenticalKindStreams() async throws {
        ...
        #expect(first == second)
        #expect(first == [.runtimeInitialized, .runtimeStarted, .goalSubmitted, .goalActivated, .goalBlocked, .goalActivated, .goalCompleted, .runtimeStopped])
    }
    ...
    @Test func coordinationPortsRemainUninvoked() async throws {
        ...
        #expect(ports.isWiredForModules == false)
        let runtime = try await AgentRuntime(identity: AgentIdentity(displayName: "E"), eventLog: InMemoryEventLog(), coordination: ports)
        try await runtime.start()
...
private func playStandardSequence() async throws -> [ExecutionEventKind] {
    let log = InMemoryEventLog()
    let runtime = try await AgentRuntime(identity: AgentIdentity(id: AgentID(rawValue: "same"), displayName: "Same"), eventLog: log, sessionTrace: TraceID(rawValue: "same.trace"))
    try await runtime.start()

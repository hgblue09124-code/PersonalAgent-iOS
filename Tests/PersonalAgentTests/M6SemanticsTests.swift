...
struct M6SemanticsTests {
    ...
    @Test func test1_ExplicitToolIDEndToEndWithoutInference() async throws {
        ...
        let explicitToolID = ToolID(rawValue: "echo")
        ...
        #expect(proposed?.payload["toolID"] == "echo")
        #expect(authorized?.payload["toolID"] == "echo")
        #expect(executed?.payload["toolID"] == "echo")
    }

    // 2. REAL Concurrent Goal Execution
    @Test func test2_RealConcurrentGoalExecution() async throws {
        let root1 = try await M6CompositionRoot(identity: AgentIdentity(displayName: "Agent 1"))
        let root2 = try await M6CompositionRoot(identity: AgentIdentity(displayName: "Agent 2"))
    ...
    @Test func test3_CausalAuditChainWithIdentityLinkage() async throws {
        ...
        let allEvents = await root.eventLog.allEvents()
    ...
    // 7. Multi-cycle Continuation Loop

...
public actor SimulatedLatencyCloudStore<Record: StorageRecord>: CloudStore {
    ...
    public init(provider: any CloudStorageProvider, yieldsPerOperation: Int = 10) {
        self.provider = provider
...
/// Actor-isolated synchronization gate using Swift concurrency checked continuations for deterministic sequencing.
...
/// Minimal conflict resolution helper for M5.5 performance tests
private func defaultM55ConflictResolver(local: MemoryStorageRecord, remote: MemoryStorageRecord, policy: ConflictResolution) throws -> MemoryStorageRecord {
...
struct M5HarnessAndPerformanceTests {
    ...
    // 1. Concurrent Sync Operations and Late Enqueueing

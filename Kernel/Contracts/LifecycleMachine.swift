...
public enum LifecycleMachine: Sendable {
    ...
    public static func apply(_ state: AgentLifecycle, command: RuntimeCommand) -> Result<AgentLifecycle, KernelError> {
        ...
        case (.paused, .resume): return .success(.running)
        case (.paused, .stop): return .success(.stopped)
        default: return .failure(.invalidLifecycleTransition(from: state, command: command))
        }
    }
    public static func eventKind(for command: RuntimeCommand) -> ExecutionEventKind {
        switch command { case .start: return .runtimeStarted; case .pause: return .runtimePaused; case .resume: return .runtimeResumed; case .stop: return .runtimeStopped }
    }
    public static func phase(for lifecycle: AgentLifecycle) -> AgentPhase {
        switch lifecycle { case .failed: return .failed; case .stopped: return .completed; case .created, .starting, .running, .pausing, .paused, .stopping: return .idle }
    }
}
